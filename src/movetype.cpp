...
/**
 * Merges the given config over the existing costs. After calling this function,
 * the caller must call clear_cache on any terrain_info that uses this one as a
 * fallback.
 *
 ...
 * @param[in] overwrite   If true, the new values overwrite the old.
 *                        If false, the new values are added to the old.
 * @param[in] siblings    Other instances that may be affected.
 */
void movetype::terrain_info::merge(const config & new_values, bool overwrite,
	const std::vector<movetype::terrain_info * > & siblings)
{
	...
	for (auto & sibling : siblings) {
		sibling->make_data_writable();
		// The call to unique_data_->merge will automatically clear
		// data_'s cache, but not the siblings' caches.
		sibling->clear_cache();
	}
...
/**
 * terrain_costs's virtual method for getting an instance that doesn't depend
 * on the lifespan of a terrain_defense or movetype object.  This will do a
 * deep copy of the data (with fallback_ already merged) if needed.
 */
...
/**
 * Ensures our data is not shared.
 */
...
/**
 * Ensures our data is immutable via any of the copies.
 */
...
/**
 * Move constructor, this is currently almost as expensive as the copy
 * constructor, because terrain_info does not have a move constructor.
 */
...
void movetype::merge(const config & new_cfg, const std::string & applies_to, bool overwrite)
{
	if (applies_to == "movement_costs") {
		...
		vision_.merge(new_cfg, overwrite, {&movement_, &jamming_});
	}
	else if (applies_to == "jamming_costs") {
		jamming_.merge(new_cfg, overwrite, {&movement_, &vision_});
	}
