...
{
	level_types_ = {
		{ng::level_type::type::scenario, _("Scenarios")},
		{ng::level_type::type::campaign, _("Multiplayer Campaigns")},
		{ng::level_type::type::sp_campaign, _("Singleplayer Campaigns")},
		{ng::level_type::type::user_map, _("Custom Maps")},
		{ng::level_type::type::user_scenario, _("Custom Scenarios")},
		{ng::level_type::type::random_map, _("Random Maps")},
	};
...
void mp_create_game::pre_show(window& win)
{
	...
	auto get_initial_type_index = [this]()->int {
		const auto index = std::find_if(level_types_.begin(), level_types_.end(), [](level_type_info& info) {
			return info.first == *ng::level_type::get_enum(preferences::level_type());
		});
	...
	//
	static const std::array<t_string, ng::random_faction_mode::size()> names {{_("Independent"), _("No Mirror"), _("No Ally Mirror")}};
	static const std::array<t_string, ng::random_faction_mode::size()> tooltips {{
	...
	for(std::size_t i = 0; i < ng::random_faction_mode::size(); i++) {
		rfm_options.emplace_back("label", names[i]);
		rfm_options[i]["tooltip"] = tooltips[i];
	};

	const int initial_index = static_cast<int>(ng::random_faction_mode::get_enum(prefs::random_faction_mode()).value_or(ng::random_faction_mode::type::independent));
...
#undef UPDATE_ATTRIBUTE
	...
	plugins_context_->set_callback("select_type",  [this](const config& cfg) {
		create_engine_.set_current_level_type(ng::level_type::get_enum(cfg["type"].str()).value_or(ng::level_type::type::scenario)); }, true);
	...
	plugins_context_->set_accessor("get_selected", [this](const config&) {
		...
		return config {
			...
			"type", ng::level_type::get_string(create_engine_.current_level_type()),
		};
	});
	...
	plugins_context_->set_accessor("find_level",   [this](const config& cfg) {
		...
		return config {
			...
			"type", ng::level_type::get_string(create_engine_.find_level_by_id(id).first),
		};
	});
...
void mp_create_game::display_games_of_type(ng::level_type::type type, const std::string& level)
{
	...
	for(const auto& game : create_engine_.get_levels_by_type_unfiltered(type)) {
		...
		if(type == ng::level_type::type::campaign || type == ng::level_type::type::sp_campaign) {
	...
	const bool is_random_map = type == ng::level_type::type::random_map;
...
void mp_create_game::update_details()
{
	...
	if(create_engine_.current_level_type() == ng::level_type::type::random_map) {
	...
	switch(create_engine_.current_level_type()) {
		case ng::level_type::type::scenario:
		case ng::level_type::type::user_map:
		case ng::level_type::type::user_scenario:
		case ng::level_type::type::random_map: {
		...
		case ng::level_type::type::campaign:
		case ng::level_type::type::sp_campaign: {
...
void mp_create_game::post_show(window& window)
{
	...
	if(get_retval() == retval::OK) {
		...
		if(create_engine_.current_level_type() == ng::level_type::type::campaign ||
			create_engine_.current_level_type() == ng::level_type::type::sp_campaign) {
			create_engine_.prepare_for_campaign();
		} else if(create_engine_.current_level_type() == ng::level_type::type::scenario) {
		...
		auto type = ng::random_faction_mode::get_enum(selected_rfm_index_).value_or(ng::random_faction_mode::type::independent);
		...
		prefs::set_random_faction_mode(ng::random_faction_mode::get_string(type));
