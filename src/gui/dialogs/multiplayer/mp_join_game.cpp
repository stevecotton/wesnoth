...
bool mp_join_game::show_flg_select(int side_num, bool first_time)
{
	if(const config& side_choice = get_scenario().child("side", side_num - 1)) {
		...
		// TODO: `saved_game` is a bad name for the type enum variable
		const auto saved_game = ng::saved_game_mode::get_enum(level_.child("multiplayer")["savegame"].str()).value_or(ng::saved_game_mode::type::no);

		ng::flg_manager flg(era_factions, side_choice, lock_settings, use_map_settings, saved_game == ng::saved_game_mode::type::midgame);
