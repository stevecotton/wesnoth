...
void campaign_selection::sort_campaigns(campaign_selection::CAMPAIGN_ORDER order, bool ascending)
{
	...
	auto levels = engine_.get_levels_by_type_unfiltered(ng::level_type::type::sp_campaign);
...
void campaign_selection::pre_show(window& window)
{
	...
	for(const auto& level : engine_.get_levels_by_type_unfiltered(ng::level_type::type::sp_campaign)) {
