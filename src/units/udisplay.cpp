		...
		display& disp)
{
	...
	animator.replace_anim_if_invalid(temp_unit,"movement",a,b,step_num,
			false,"",{0,0,0},attack_result::type::invalid,nullptr,nullptr,step_left);
	animator.start_animations();
		...
		const_attack_ptr attack,const_attack_ptr secondary_attack, const map_location& defender_loc, unit_ptr defender)
{
	...
	animator.add_animation(attacker.shared_from_this(),"draw_weapon",loc,defender_loc,0,true,"",{0,0,0},attack_result::type::hit,attack,secondary_attack,0);
	if(defender) {
		animator.add_animation(defender,"draw_weapon",defender_loc,loc,0,true,"",{0,0,0},attack_result::type::miss,secondary_attack,attack,0);
	}
	animator.start_animations();
	animator.wait_for_end();
		...
		const_attack_ptr primary_attack,const_attack_ptr secondary_attack, const map_location& secondary_loc,unit_ptr secondary_unit)
{
	...
	unit_animator animator;
	if(primary_unit) {
		animator.add_animation(primary_unit,"sheath_weapon",primary_loc,secondary_loc,0,true,"",{0,0,0},attack_result::type::invalid,primary_attack,secondary_attack,0);
	}
	if(secondary_unit) {
		animator.add_animation(secondary_unit,"sheath_weapon",secondary_loc,primary_loc,0,true,"",{0,0,0},attack_result::type::invalid,secondary_attack,primary_attack,0);
	}
		...
		const_attack_ptr attack,const_attack_ptr secondary_attack, const map_location& winner_loc, unit_ptr winner)
{
	...
	animator.add_animation(loser.shared_from_this(),"death",loc,winner_loc,0,false,"",{0,0,0},attack_result::type::kill,attack,secondary_attack,0);
	...
	if(winner) {
		animator.add_animation(winner,"victory",winner_loc,loc,0,true,"",{0,0,0},
			attack_result::type::kill,secondary_attack,attack,0);
	}
	animator.start_animations();
	animator.wait_for_end();
                 ...
                 bool attacking)
{
	...
	attack_result::type hit_type;
	if(damage >= defender.hitpoints()) {
		hit_type = attack_result::type::kill;
	} else if(damage > 0) {
		hit_type = attack_result::type::hit;
	}else {
		hit_type = attack_result::type::miss;
	}
